...
export default function Usuarios() {
  ...
  const openEdit = (u: any) => {
    ...
    setEditForm({
      ...
      user_group: (u as any).user_group ?? "cya",
    });
    setEditOpen(true);
  };
  ...
  const handleCreate = async () => {
    ...
    try {
      await createUser.mutateAsync({
        ...
        user_group: createForm.user_group,
  ...
  const countByGroup = (group: string) =>
    (users ?? []).filter((u) => ((u as any).user_group ?? "cya") === group).length;
        ...
        <DialogContent className="sm:max-w-md">
          ...
          {editUser && (
            <div className="grid gap-4 py-2">
              ...
              <div className="space-y-2">
                <Label>Grupo</Label>
                <Select value={editForm.user_group} onValueChange={(v) => setEditForm((p) => ({ ...p, user_group: v }))}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
              ...
              {["b2b_cliente", "cliente_b2c"].includes(editForm.role) && (
                <div className="space-y-2">
                  ...
                  <Select value={editForm.company_id} onValueChange={(v) => setEditForm((p) => ({ ...p, company_id: v }))}>
              ...
              <div className="flex items-center gap-2">
                <Checkbox
                  ...
                  onCheckedChange={(c) => setEditForm((p) => ({ ...p, is_active: !!c }))}
                />
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="grid gap-4 py-2">
            ...
            <div className="space-y-2">
              <Label>Email *</Label>
              <Input type="email" value={createForm.email} onChange={(e) => setCreateForm((p) => ({ ...p, email: e.target.value }))} placeholder="email@exemplo.com" />
            </div>
            <div className="space-y-2">
              <Label>Senha provisória *</Label>
              <Input type="password" value={createForm.password} onChange={(e) => setCreateForm((p) => ({ ...p, password: e.target.value }))} placeholder="Mín. 6 caracteres" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-2">
                <Label>Grupo *</Label>
                <Select value={createForm.user_group} onValueChange={(v) => setCreateForm((p) => ({ ...p, user_group: v }))}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {Object.entries(GROUP_LABELS).map(([k, v]) => (
                      <SelectItem key={k} value={k}>{v}</SelectItem>
            ...
            {["b2b_cliente", "cliente_b2c"].includes(createForm.role) && (
              <div className="space-y-2">
                <Label>Empresa Vinculada</Label>
                <Select value={createForm.company_id} onValueChange={(v) => setCreateForm((p) => ({ ...p, company_id: v }))}>
                  <SelectTrigger><SelectValue placeholder="Selecione..." /></SelectTrigger>
                  <SelectContent>
                    {(companies ?? []).map((c) => (
                      <SelectItem key={c.id} value={c.id}>{c.company_name}</SelectItem>
            ...
            <div className="flex items-center gap-2">
              <Checkbox
                ...
                onCheckedChange={(c) => setCreateForm((p) => ({ ...p, is_active: !!c }))}
              />
