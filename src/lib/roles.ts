export type AppRole = "admin" | "gestao" | "gerente_operacional" | "loja" | "producao" | "compras" | "b2b_cliente";

export const ROLE_ROUTES: Record<AppRole, string[]> = {
  admin: [
    "/", "/dre", "/contas-pagar", "/fluxo-caixa", "/dashboard-financeiro", "/produtos", "/clientes-b2b", "/clientes-b2c", "/usuarios", "/compras", "/producao", "/b2b", "/b2b/pedidos",
    "/op", "/op/producao-dia", "/op/estoque-loja", "/op/conciliacao", "/op/aprovacoes", "/op/programadas",
    "/central-operacional", "/central-b2b",
    "/custeio/parametros", "/custeio/insumos", "/custeio/fichas",
    "/conselho",
  ],
  gestao: [
    "/", "/dre", "/contas-pagar", "/fluxo-caixa", "/dashboard-financeiro", "/produtos", "/clientes-b2b", "/clientes-b2c", "/usuarios", "/compras", "/producao", "/b2b", "/b2b/pedidos",
    "/op", "/op/producao-dia", "/op/estoque-loja", "/op/conciliacao", "/op/aprovacoes", "/op/programadas",
    "/central-operacional", "/central-b2b",
    "/custeio/parametros", "/custeio/insumos", "/custeio/fichas",
    "/conselho",
  ],
  gerente_operacional: [
    "/op", "/op/producao-dia", "/op/estoque-loja", "/op/conciliacao", "/op/aprovacoes", "/op/programadas",
    "/central-operacional", "/central-b2b",
    "/custeio/parametros", "/custeio/insumos", "/custeio/fichas",
    "/produtos", "/producao", "/clientes-b2b", "/b2b", "/b2b/pedidos",
  ],
  loja: ["/op/estoque-loja", "/central-operacional", "/clientes-b2c"],
  producao: ["/op/producao-dia", "/central-operacional", "/central-b2b", "/producao"],
  compras: ["/compras"],
  b2b_cliente: ["/b2b", "/b2b/pedidos"],
};

export const ROLE_DEFAULT_ROUTE: Record<AppRole, string> = {
  admin: "/",
  gestao: "/",
  gerente_operacional: "/central-operacional",
  loja: "/central-operacional",
  producao: "/central-operacional",
  compras: "/compras",
  b2b_cliente: "/b2b",
};

export function canAccess(role: string, path: string): boolean {
  const routes = ROLE_ROUTES[role as AppRole];
  if (!routes) return false;
  return routes.includes(path);
}

export function getDefaultRoute(role: string): string {
  return ROLE_DEFAULT_ROUTE[role as AppRole] ?? "/";
}
